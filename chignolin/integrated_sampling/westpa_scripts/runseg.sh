...
ln -sv $WEST_SIM_ROOT/gromacs_config/charmm22star.ff .
ln -sv $WEST_SIM_ROOT/gromacs_config/topol_01.top .
ln -sv $WEST_SIM_ROOT/gromacs_config/chignolin-ref.pdb .
...
if [ "$WEST_CURRENT_SEG_INITPOINT_TYPE" = "SEG_INITPOINT_CONTINUES" ]; then
  ...
  gmx_mpi grompp -f md.mdp -c parent.gro -e parent.edr -p topol_01.top \
...
# Now take care of the case that the trajectory is starting anew.
  ...
  ln -sv $WEST_SIM_ROOT/gromacs_config/folded.gro ./parent.gro

  # Run the GROMACS preprocessor
  gmx_mpi grompp -f md.mdp -c parent.gro -p topol_01.top \
